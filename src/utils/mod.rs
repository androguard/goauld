...
const TMP_DIR_PATH: &str = "/tmp";
#[cfg(target_os = "android")]
const TMP_DIR_PATH: &str = "/data/local/tmp";


#[cfg(target_os = "linux")]
pub fn get_dlopen_lib_name() -> String {
    "libc.so".to_string()
}
...
pub fn hexdumpm(buffer: &[u8], max_lines_opt: Option<usize>, writer: &mut Vec<u8>) -> Result<(), InjectionError>
{
...
fn hex(line: usize, parts: &[u8], writer: &mut Vec<u8>) -> Result<(), InjectionError>
{
    if line > 0 {
        writeln!(writer)?;
    }
...
pub fn hexdump(buffer: &[u8], writer: &mut Vec<u8>) -> Result<(), InjectionError>
{
